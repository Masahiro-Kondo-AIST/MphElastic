...
public:
    mat3():a00((T)0),a01((T)0),a02((T)0),a10((T)0),a11((T)0),a12((T)0),a20((T)0),a21((T)0),a22((T)0){}
 ...
 template<typename U> explicit mat3(const U * const m):a00((T)m[3*0 +0]), a01((T)m[3*0 +1]), a02((T)m[3*0 +2]), a10((T)m[3*1 +0]), a11((T)m[3*1 +1]), a12((T)m[3*1 +2]), a20((T)m[3*2 +0]), a21((T)m[3*2 +1]), a22((T)m[3*2 +2]){}
    mat3(const T& s):a00((T)s),a01((T)0),a02((T)0),a10((T)0),a11((T)s),a12((T)0),a20((T)0),a21((T)0),a22((T)s){}
 ...
 const T* c_array()const{ return a[0]; }
    ...
    T trace(){return a00 + a11 + a22;}
    mat3 trans(){return mat3(a00,a10,a20,a01,a11,a21,a02,a12,a22);}
    T det(){return a00*a11*a22 + a01*a12*a20 + a02*a10*a21 - a00*a12*a21 - a01*a10*a22 - a02*a11*a20;}
    mat3 adjoint(){return mat3(a11*a22-a12*a21,a12*a20-a10*a22,a10*a21-a11*a20,a21*a02-a22*a01,a22*a00-a20*a02,a20*a01-a21*a00,a01*a12-a02*a11,a02*a10-a00*a12,a00*a11-a01*a10);}
    mat3 inverse(){assert((*this).det()!=0);return 1.0/(*this).det() * mat3(a11*a22-a12*a21,a21*a02-a01*a22,a01*a12-a11*a02,a12*a20-a22*a10,a22*a00-a20*a02,a02*a10-a12*a00,a10*a21-a20*a11,a20*a01-a00*a21,a00*a11-a01*a10);}
...
template <typename T>
inline mat3<T> operator % (const vec3<T>& vl, const vec3<T>& vr){ return mat3<T>( vl.b[0]*vr.b[0], vl.b[0]*vr.b[1], vl.b[0]*vr.b[2], vl.b[1]*vr.b[0], vl.b[1]*vr.b[1], vl.b[1]*vr.b[2], vl.b[2]*vr.b[0], vl.b[2]*vr.b[1], vl.b[2]*vr.b[2]);}
